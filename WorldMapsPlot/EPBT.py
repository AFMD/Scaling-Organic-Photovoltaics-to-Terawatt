...
def setup_configuration():
    ...
    return {
        ...
        'temp_coeff': 0.34,  # %/K
        'years': [2024, 2040],
        'save_path': 'EPBT_{mode}_{year}.png'
    }
...
def calculate_technology_parameters(index):
    ...
    CED_OPV = CED_module_OPV[index] / 3.6
...
def create_visualization(result, extent, config):
    ...
    cbar.set_label(f'EPBT - {config["mode"]} in {config["years"][config["index"]]}',
                   rotation=0, labelpad=-100, va='top', fontsize=28,
...
def main():
    ...
    # Print statistics
    print('Min/Max EPBT:', np.min(result), np.max(result))
    print('Mean EPBT:', np.mean(result))
